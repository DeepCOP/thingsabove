alter table public.plan_submissions
  drop constraint if exists plan_submissions_status_check;

alter table public.plan_submissions
  add constraint plan_submissions_status_check
  check (status in ('submitted', 'screening', 'approved', 'published', 'rejected', 'failed'));
