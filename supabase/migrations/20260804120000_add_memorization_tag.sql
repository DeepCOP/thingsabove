...
-- Plans tagged Memorization intentionally use the same scripture-required, notes-optional behavior.
create or replace function public.tags_include_scripture_focused(tags text[])
returns boolean
language sql
immutable
set search_path = pg_catalog, public
as $$
  select exists (
    ...
    where lower(btrim(tag.value)) in ('reading', 'memorization')
  );
$$;
...
begin
  ...
  v_is_reading_plan := public.tags_include_scripture_focused(v_plan.tags);
...
begin
  ...
  v_is_reading_plan := public.tags_include_scripture_focused(v_submission.submitted_tags);
...
drop function if exists public.tags_include_reading(text[]) restrict;
